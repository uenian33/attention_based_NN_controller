...
class LinearAttentionBlock(nn.Module):
    ...
    def __init__(self, in_features, normalize_attn=True, linear_fusion=False, final_output=True):
...
class SensorMaskBlock(nn.Module):
    """docstring for SensorMaskBlock"""

    def __init__(self, x_dim):
        super(SensorMaskBlock, self).__init__()
...
class DualMaskBlock(nn.Module):
    ...
    def forward(self, sens_mask, act_mask, x):
        weighted_x = torch.mul(sens_mask, x)
        x = self.reg_layers(weighted_x)
        x = act_mask * x
        return x
